  ...
  return (
    <div className="min-h-screen py-10 px-4 relative z-10">
      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold" style={{ color: 'var(--ah-ink)' }}>
            İletişim
          </h1>
        ...
        <section className="bg-white rounded-2xl shadow-xl border border-brand-200 p-6 sm:p-8 mb-6">
          <div className="text-[10px] uppercase tracking-widest font-bold mb-3" style={{ color: 'var(--ah-ink-3)' }}>
            Veri Sorumlusu
          </div>
        ...
        <section className="bg-white rounded-2xl shadow-xl border border-brand-200 p-6 sm:p-8">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-[12px] font-semibold mb-1.5" style={{ color: 'var(--ah-ink-3)' }}>
                Konu
              </label>
              <select value={subject} onChange={e => setSubject(e.target.value)}
                className="w-full px-4 py-2.5 rounded-xl text-sm border focus:outline-none focus:ring-2"
                style={{ background: 'var(--ah-card)', borderColor: 'var(--ah-line)', color: 'var(--ah-ink)' }}>
            ...
            <div>
              ...
              <textarea
                ...
                style={{ background: 'var(--ah-card)', borderColor: 'var(--ah-line)', color: 'var(--ah-ink)', minHeight: 140 }}
              />
            </div>
            {sent ? (
              <div style={{
                ...
                color: 'var(--ah-ink-2)', fontSize: 13, fontWeight: 600,
                textAlign: 'center',
              }}>
              ...
              <>
                <button type="button" onClick={submitToBackend}
                  ...
                  style={{
                    ...
                    color: body.trim() && !sending ? '#fff' : 'var(--ah-ink-3)',
                    border: 'none',
              ...
              <>
                <a href={mailtoHref}
                   className="block w-full text-center px-5 py-3 rounded-xl font-semibold text-sm transition-all"
                   style={{
                     background: body.trim() ? 'var(--ah-brand-gradient)' : 'rgba(136, 136, 136, 0.18)',
                     color: body.trim() ? '#fff' : 'var(--ah-ink-3)',
...
function InfoCard({ label, value, href, caption }) {
  const inner = (
    <>
      <div className="text-[10px] uppercase tracking-widest" style={{ color: 'var(--ah-ink-3)' }}>{label}</div>
      <div className={`font-mono text-[13px] truncate mt-1 ${href ? 'underline' : ''}`}
           style={{ color: 'var(--ah-ink)' }}>{value}</div>
      {caption && (
        <div className="text-[10px] mt-1" style={{ color: 'var(--ah-ink-4)' }}>{caption}</div>
      )}
    </>
  )
