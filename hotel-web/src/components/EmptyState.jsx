    ...
    <div className={`flex flex-col items-center text-center ${compact ? 'py-8' : 'py-12'} px-4`}>
      ...
      {title && (
        <h3 className="text-xl font-bold mb-2"
            style={{ color: 'var(--ah-ink)', letterSpacing: '-0.02em' }}>
          {title}
        </h3>
      )}
      {description && (
        <p className="text-sm max-w-md mb-6 leading-relaxed" style={{ color: 'var(--ah-ink-3)' }}>
          {description}
        </p>
      )}
      ...
      {normalizedSteps && normalizedSteps.length > 0 && (
        <ol className="w-full max-w-md text-left mb-6 space-y-2">
          {normalizedSteps.map((s, i) => (
            <li key={i} className="flex items-start gap-3 rounded-lg px-3.5 py-3"
                style={{
                  background: 'var(--ah-page)',
                  border: '1px solid var(--ah-line)',
                }}>
              <span className="text-[14px] font-bold tabular-nums flex-shrink-0 w-7 text-center"
                    style={{ color: 'var(--ah-brand)', letterSpacing: '-0.005em' }}>
                {String(i + 1).padStart(2, '0')}
              </span>
              <div className="flex-1 min-w-0">
                <div className="text-[13px] font-semibold" style={{ color: 'var(--ah-ink)' }}>
                  {s.label}
                </div>
                {s.hint && (
        ...
        <div className="flex flex-wrap gap-2 justify-center">
          {ctaLabel && (
            <button onClick={onCta}
              className="px-5 py-2.5 rounded-lg text-[13px] font-semibold transition-opacity hover:opacity-90"
